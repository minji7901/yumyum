...
export const calculateTotalNutrients = (data: { total_nutritions: SumNutrients }[]): SumNutrients => {
  if (!Array.isArray(data)) {
    ...
    return {} as SumNutrients; 
  }

  const totalNutrients = data.reduce((acc, curr) => {
    const nutrients = curr.total_nutritions;
    // 현재 객체의 total_nutritions 내 항목을 순회하여 합산
    for (const key in nutrients) {
      acc[key as keyof SumNutrients] = (acc[key as keyof SumNutrients] || 0) + nutrients[key as keyof SumNutrients];
    }
    return acc;
  }, {} as SumNutrients);
